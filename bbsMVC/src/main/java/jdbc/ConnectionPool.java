...
//ConnectionFactory로 부터 maxConn 만큼 conn 객체를 생성해서 풀(pool)에 보관하는 클래스
public class ConnectionPool {
	...
	//1. 풀에서 사용가능한 연결을 가져오는 메서드, 사용가능한 연결이 없을경우wait()
	...
	private synchronized void destroyPool() throws SQLException {
		for(int i=0; i<pool.size(); i++) {
			pool.get(i).close(); //connection 닫기
		}
		pool.clear();
	}
	//4.최초에 초기화하는 메서드
